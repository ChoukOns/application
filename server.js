...
app.post('/api/cours', async (req, res) => {
    try {
        ...
        const { titre, fichier, prix, langue, domaine } = req.body;
...
app.post('/api/:id_cours/commentaires', async (req, res) => {
    try {
        ...
        const { commentaire, apprenantId } = req.body; // Ajoutez apprenantId dans le corps de la requête

        // Créez un nouveau commentaire avec l'ID de l'apprenant
        const nouveauCommentaire = await Commentaire.create({ id_cours, commentaire, apprenant: apprenantId });

        // Récupération de toutes les données de l'apprenant
